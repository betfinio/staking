...
export const CycleOverview: FC = () => {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <div
        className={
          'col-span-2 md:col-span-1 p-3 md:p-5 relative  bg-primaryLighter border border-gray-800 rounded-lg flex justify-between flex-col gap-4'
        }
      >
        <Tooltip>
          <TooltipTrigger className={'absolute  top-3 right-3'}>
            <CircleHelp
              ...
              onClick={handleCalculateOld}
            />
          </TooltipTrigger>
          <TooltipContent
            className={
              'bg-black px-4 py-2 rounded-md border border-yellow-400 text-white w-[350px]'
            }
          >
            <div className={'text-sm'}>
              <b className={'font-semibold text-yellow-400'}>Dynamic staking cycle</b> is a period of <b
              className={'font-semibold text-yellow-400'}>4 weeks</b> when players bet
              against stakers in <b className={'font-semibold text-yellow-400'}>dynamic games</b> and generate <b
              className={'font-semibold text-yellow-400'}>Profit and
              Loss</b> (PnL) to dynamic staking.

              <br/>
              <br/>
              Every <b className={'font-semibold text-yellow-400'}>4th Monday at 12.00 UTC</b> there is a calculation
              window when the PnL is distributed among all the <b
              className={'font-semibold text-yellow-400'}>dynamic pools.</b>
              <br/>
              <br/>
              Staking rewards distribution is done automatically without claim, staking in drawdowns (when users won
              more than lost in a cycle) does not receive payout till the
              drawdown is covered.
        ...
        <h1 className={'text-left font-normal text-sm lg:text-base md:font-semibold'}>
          Dynamic cycle <span className={'underline cursor-pointer'}
        ...
        <div
          className={'md:grid grid-cols-1 sm:grid-cols-3 gap-2 lg:gap-4 hidden'}
        >
...
export const CycleProgress: FC<{ start: number; end: number }> = ({
                                                                    start,
                                                                    end,
                                                                  }) => {
...
const NewStakes: FC<{ cycleStart: number }> = ({ cycleStart }) => {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        className={
          'bg-black text-white p-2 text-sm border-yellow-400 border rounded-md'
        }
      >
  ...
  return (
    <Tooltip>
      <TooltipTrigger>
        <div
          className={
            'border border-green-500 py-4 px-3 rounded-md bg-primary text-sm text-[#6A6F84]'
          }
        >
          <p className={'font-semibold text-center leading-[10px]'}>Cycle revenues</p>
          <div className={'mt-5'}>
            <div className={'flex justify-center font-bold text-green-500'}>{profit >= 0 ? '+' : ''}<MillifyWithTooltip
              value={valueToNumber(profit)} withIcon={true}/></div>
            <p className={'text-center '}>({profit >= 0 ? '+' : '-'}{percentage.toFixed(2)}%)</p>
          </div>
