...
const StakingInfo: FC = () => {
  ...
  const { data: totalVolume = 0n, isLoading: isTotalVolumeLoading } =
    useTotalVolume();
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div
          className={
            'bg-primaryLighter border border-gray-800 rounded-lg cursor-pointer'
          }
        >
          <div
            className={
          ...
          <div className={'rounded-lg flex justify-between font-semibold p-3'}>
            ...
            >
              ...
              <div
                className={
                  'flex flex-col gap-[18px] md:gap-4 justify-between items-start h-full'
                }
              >
                ...
                <p
                  className={cx(
                    ...
                    { 'animate-pulse blur-sm': isTotalBetsLoading },
                  )}
                >
                  <Blackjack
                  ...
                  />
                  {t('total.bets', {
                    count: isTotalBetsLoading ? 12345 : totalBets,
                  })}
                </p>
                <div className={cx('flex flex-row items-center gap-2')}>
                  ...
                  <BetValue
                    value={valueToNumber(
                      isTotalVolumeLoading ? 123456n * 10n ** 18n : totalVolume,
                    )}
                    precision={2}
                    withIcon
                  />{' '}
            ...
            <div className={'text-center flex flex-col justify-center '}>
              ...
              <div
                className={cx(
                  'mt-3 flex flex-row items-center gap-1  mx-auto',
                  { 'animate-pulse blur-sm': isTotalProfitLoading },
                  { 'text-green-400': isTotalProfitFetching || glow },
                )}
              >
                {Math.floor(
                  valueToNumber(
                    isTotalProfitLoading ? 1234567n * 10n ** 18n : totalProfit,
                  ) * share,
                ).toLocaleString()}
      ...
      <DialogContent>
        <motion.div
          className={cx(
            'rounded-lg bg-primaryLighter z-10 border border-gray-800 font-semibold text-white mx-auto max-w-[500px]',
          )}
        >
          <div className={'py-5 px-7'}>
            ...
            <p className={'flex items-center justify-center gap-2 '}>
              ...
              <span className={'font-normal sm:hidden'}>
                {truncateEthAddress(DYNAMIC_STAKING_ADDRESS)}
              </span>
              <Link
                to={
                ...
                target={'_blank'}
                className={'font-normal hidden sm:inline underline'}
              >
