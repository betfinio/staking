...
import {useStaked, useTotalBets, useTotalStaked, useTotalVolume} from "@/src/lib/query/conservative";
...
const StakingInfo: FC = () => {
	...
	const {data: totalVolume = 0n, isLoading: isTotalStakedLoading} = useTotalVolume()
	const {data: totalBets = 0, isLoading: isTotalBetsLoading} = useTotalBets()
	const {data: currentBalance = 0n, isFetching: isTotalProfitFetching, isLoading: isTotalProfitLoading} = useBalance(import.meta.env.PUBLIC_CONSERVATIVE_STAKING_ADDRESS);
	...
	const to = useMemo(() => {
		const value = Math.floor(valueToNumber(isTotalProfitLoading ? 0n : currentBalance) * (isNaN(share) ? 0 : share))
		setTimeout(() => {
			setFrom(value)
		}, 1000)
		return value
	...
	return <div className={'cursor-pointer bg-primaryLighter border border-gray-800 rounded-lg'}>
		...
		<div className={'rounded-lg flex justify-between font-semibold p-3'}>
			...
			<div className={'text-center flex flex-col justify-center '}>
				<ConservativeStaking className={cx('mx-auto w-12 text-yellow-400 duration-300 ', {"!animate-pulse text-green-400 ": isTotalProfitFetching || glow})}/>
