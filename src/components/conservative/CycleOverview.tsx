...
import cx from "clsx";
import {useCalculateProfit, useTotalProfitDiff, useTotalStakedDiff} from "@/src/lib/query/conservative";
...
export const CycleOverview: FC = () => {
	...
	const {mutate: calculate} = useCalculateProfit();
				...
				<Tooltip>
					...
					<TooltipContent className={'bg-black px-4 py-2 rounded-md border border-yellow-400 text-white w-[350px]'}>
						<div className={'text-sm'}>
							<b className={'font-semibold text-yellow-400'}>Conservative staking cycle</b> is a period of <b className={'font-semibold text-yellow-400'}>1 week</b> when players
							can join the <b className={'font-semibold text-yellow-400'}>conservative games</b> and generate <b className={'font-semibold text-yellow-400'}> Profit </b> from
							winning fees to conservative staking.
							
							<br/>
							<br/>
							Every <b className={'font-semibold text-yellow-400'}>Friday at 12:00 UTC</b> there is a calculation window when the profit from winning fees is distributed among
							all the <b className={'font-semibold text-yellow-400'}>conservative staking pools</b>.
							<br/>
							<br/>
							All the stakers have to <b className={'font-semibold text-yellow-400'}>claim</b> those rewards afterwards.
							<br/>
							<br/>
							There <b className={'font-semibold text-yellow-400'}>can not be a negative payout</b> and therefore there is no possibility to lost tokens in conservative staking.
						
						</div>
					</TooltipContent>
				</Tooltip>
				<h1 className={'text-left font-normal text-sm lg:text-base md:font-semibold'}>
					Conservative cycle <span className={'underline cursor-pointer'}
					                         onClick={handleCalculate}>#{cycleId}</span> overview
				</h1>
				<div>
				...
				<div className={'md:hidden'}>
					<SharedGameBlock games={[
						...
						},
						{
							...
							subtitle: `(${isNaN(diff) ? "0" : diff.toFixed(2)}%)`
						},
						{
							amount: 0n,
							label: "Ending stakes",
			...
			<div className={'h-[20px] w-full bg-primary'}>
				...
				>
					<motion.p initial={{opacity: 0}} animate={{opacity: 1}} transition={{delay: 1}}
					          className={cx('absolute top-1/2 -translate-y-1/2 text-[10px] font-bold whitespace-nowrap', progress > 50 ? 'right-0 pr-2 text-primary' : 'right-0 translate-x-[100%] pl-2')}>
						{days > 0 ? (
...
const NewStakes: FC<{ cycleStart: number }> = ({cycleStart}) => {
	...
	return (
		<Tooltip>
			<TooltipTrigger>
				<div
					className={'shrink-0 border border-yellow-400 py-4 px-3 rounded-md bg-primary text-sm text-[#6A6F84]'}>
					<div className={'font-semibold text-center leading-[10px]'}>New stakes</div>
					<div className={'mt-5'}>
						<div className={'flex justify-center text-yellow-400'}>+<BetValue value={valueToNumber(newStaked[0])} withIcon={true}/>
						</div>
			...
			<TooltipContent className={'bg-black text-white p-2 text-sm border-yellow-400 border rounded-md'}>
				Volume staked during current cycle
			</TooltipContent>
		</Tooltip>
	)
}
...
const GamesProfit = () => {
	...
	return (
		<Tooltip>
			<TooltipTrigger>
				<div
					className={'border border-green-500 py-4 px-3 rounded-md bg-primary text-sm text-[#6A6F84]'}>
					...
					<div className={'mt-5'}>
						...
						<div className={'text-center '}>({isNaN(diff) ? "0" : diff.toFixed(2)}%)</div>
					</div>
				</div>
			</TooltipTrigger>
			<TooltipContent className={'bg-black text-white p-2 text-sm border-yellow-400 border rounded-md'}>
				Volume to distribute between stakers
			</TooltipContent>
		</Tooltip>
	)
}
...
const EndingStakes = () => {
	// todo
	return (
		<div className={'border border-red-500 py-4 px-3 rounded-md bg-primary text-sm text-[#6A6F84]'}>
			<div className={'font-semibold text-center leading-[10px]'}>Ending stakes</div>
			<div className={'mt-5'}>
				<div className={'flex justify-center text-red-500'}><BetValue value={0} withIcon={true}/>
				</div>
